        ...
        Column(modifier = Modifier.weight(1f).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(AppDesign.spacingLarge)) {
            ...
            when (state.displayMode) {
                ...
                FourierDisplayMode.COMPLEX -> ComplexHarmonicComponents(
                    ...
                    onFrequencyChange = { i, f -> if (state.waveType == WaveType.PURE_SIGNAL) { state.customFunctionSignals[i].freq = "%.2f".format(f); state.customFunctionSignals[i].updateCache() } else state.harmonicFrequencies[i] = f; state.path.clear() },
                    ...
                    onAmplitudeChange = { i, a -> if (state.waveType == WaveType.PURE_SIGNAL) { state.customFunctionSignals[i].amp = "%.2f".format(a); state.customFunctionSignals[i].updateCache() } else state.harmonicAmplitudes[i] = a; state.path.clear() },
                    ...
                    onPhaseChange = { i, p -> if (state.waveType == WaveType.PURE_SIGNAL) { state.customFunctionSignals[i].phase = "%.2f".format(p * 180f / Math.PI.toFloat()); state.customFunctionSignals[i].updateCache() } else state.harmonicPhases[i] = p; state.path.clear() },
